// La importación de la configuración ahora usa una ruta RELATIVA correcta
...
// --- GESTIÓN DE VISTAS (BLOQUE CORREGIDO Y ROBUSTO) ---
...
function showView(viewId) {
    ...
    if (activeView) {
        ...
        if (isMenuView) {
            ...
            const titleElement = activeView.querySelector('h2');
            if (titleElement) {
                viewTitle.textContent = titleElement.textContent;
            }
        }
...
onAuthStateChanged(auth, user => {
    loadingView.classList.add('hidden'); // Ocultar carga en cualquier caso
    if (user) {
        currentUser = user;
